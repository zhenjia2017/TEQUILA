...
//		}
		...
		if(answerContext.QuestionAnwser != null && answerContext.QuestionAnwser.getDateRankSection(CGlobalConfiguration.SelectedRank ) != null){
			answerContext.QuestionAnwser.set_currentDateRankSection(CGlobalConfiguration.SelectedRank );//get current rank
			this.candidateFilter.Process(answerContext); //process answer in rank
			...
			//if has final answer, set rank mark
			if(HasFinalAnswer(answerContext)){ 
				answerContext.SentenceReport.AnswerReport.AnswerFromRank = CGlobalConfiguration.SelectedRank;
			}
		}
